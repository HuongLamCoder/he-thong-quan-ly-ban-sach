...
class PDF extends tFPDF {
    ...
    function Header() {
        ...
        //Container for image
        $this->Cell(40, 10, "", 0, 0);
        $this->Image(__DIR__.'/../../asset/quantri/img/vinabook-logo.png', 10, 11.5, 35, 5);
        //Header title
        $this->Cell(0, 10, "Công ty TNHH Tám Thành Viên Vinabook", 0, 1);
...
//Get value of opt & time from URL (month/year - income/ profit/ cost)
if(isset($_GET['month'])) {
    $date = $_GET['month'];
    $dateStart = $date . "-01";
    $dateEnd = $date . "-" . ReportFunctions::lastDayOfMonth($date);
    $weeks = ReportFunctions::getWeeks($date);
}
else if(isset($_GET['year'])) {
    $year = $_GET['year'];
    $dateStart = ReportFunctions::dateFormat($year . "-01-01");
    $dateEnd = ReportFunctions::dateFormat($year . "-12-31");
    $months = ReportFunctions::getDaysOfMonth($year);
}

if(isset($_GET['time']) && ($_GET['time'] == "month") && isset($_GET['report'])) {
    ...
    switch ($opt) {
        case 'income':
            ...
            //Title
            $pdf->Cell(0, 10, "BÁO CÁO DOANH THU THÁNG ". ReportFunctions::getMonth($date) ."", 0, 1, "C");
            $pdf->SetFont("DejaVu", "", 13);
            $pdf->Cell(0, 6, "từ ngày ".ReportFunctions::dateFormat($dateStart)." đến ngày ".ReportFunctions::dateFormat($dateEnd)."", 0, 1, "C");
            ...
            //Table
                ...
                for($i = 1; $i <= sizeof($weeks); $i++) {
                    ...
                    /* orderCount */
                    $orderCount = Order::getOrderCount($dateWeekStart, $dateWeekEnd);

                    /* orderProductCount */
                    $orderProductCount = Order::getOrderProductCount($dateWeekStart, $dateWeekEnd);

                    /* orderTotal */
                    $orderTotal = Order::getOrderTotal($dateWeekStart, $dateWeekEnd);

                    $sumOrderCount += $orderCount;
                    $sumOrderProductCount += $orderProductCount;
                    $sumOrderTotal += $orderTotal;

                    $pdf->SetFont("DejaVu", "B", 13);
                    $pdf->Cell(30, 10, "$i", 1, 0, 'C');
                    $pdf->SetFont("DejaVu", "", 13);
                    $pdf->Cell(40, 10, "".ReportFunctions::dateFormat($dateWeekStart)."", 1, 0, 'C');
                    $pdf->Cell(40, 10, "".ReportFunctions::dateFormat($dateWeekEnd)."", 1, 0, 'C');
                    $pdf->Cell(50, 10, "$orderCount", 1, 0, 'R');
                    $pdf->Cell(50, 10, "$orderProductCount", 1, 0, 'R');
                    $pdf->Cell(60, 10, "".ReportFunctions::priceFormat($orderTotal)." đ", 1, 1, 'R');
                ...
                $pdf->Cell(50, 12, "$sumOrderProductCount", 1, 0, 'R', false);
                $pdf->Cell(60, 12, "".ReportFunctions::priceFormat($sumOrderTotal)." đ", 1, 1, 'R', false);
                //End: table data
            //End: table

            // Price in words
            $pdf->SetFont("DejaVu", "I", 15);
            $pdf->Cell(270, 10, "(".ReportFunctions::numberToWords($sumOrderTotal)." đồng)", 0, 1, 'R');
            ...
            $pdf->SetFont("DejaVu", "", 13);
            $pdf->Cell(150, 8, "", 0, 0);
            $pdf->Cell(120, 8, "TP.HCM, ".ReportFunctions::getCurrentDate()."", 0, 1, 'C');
            $pdf->SetFont("DejaVu", "B", 13);
            $pdf->Cell(150, 8, "", 0, 0);
        ...
        case "cost":
            ...
            //Title
            $pdf->Cell(0, 10, "BÁO CÁO NHẬP KHO THÁNG ". ReportFunctions::getMonth($date) ."", 0, 1, "C");
            $pdf->SetFont("DejaVu", "", 13);
            $pdf->Cell(0, 6, "từ ngày ".ReportFunctions::dateFormat($dateStart)." đến ngày ".ReportFunctions::dateFormat($dateEnd)."", 0, 1, "C");
            ...
            //Table
                ...
                for($i = 1; $i <= sizeof($weeks); $i++) {
                    ...
                    $dateWeekEnd = $weeks[$i][sizeof($weeks[$i]) - 1];
                    /* grnCount */
                    $grnCount = GRN::getGRNCount($dateWeekStart, $dateWeekEnd);
                    /* grnCount */

                    /* grnQty */
                    $grnQty = GRN::getGRNProductCount($dateWeekStart, $dateWeekEnd);
                    /* grnQty */
                    
                    /* grnTotal */
                    $grnTotal = GRN::getGRNTotal($dateWeekStart, $dateWeekEnd);
                    /* grnTotal */
                    ...
                    $pdf->Cell(60, 10, "".ReportFunctions::priceFormat($grnTotal)." đ", 1, 1, 'R');
                ...
                $pdf->Cell(60, 12, "".ReportFunctions::priceFormat($sumGRNTotal)." đ", 1, 1, 'R', false);
                //End: table data
            //End: table

            // Price in words
            $pdf->SetFont("DejaVu", "I", 15);
            $pdf->Cell(270, 10, "(".ReportFunctions::numberToWords($sumGRNTotal)." đồng)", 0, 1, 'R');
        ...
        case "profit":
            ...
            //Title
            $pdf->Cell(0, 10, "BÁO CÁO LỢI NHUẬN THÁNG ". ReportFunctions::getMonth($date) ."", 0, 1, "C");
            $pdf->SetFont("DejaVu", "", 13);
            $pdf->Cell(0, 6, "từ ngày ".ReportFunctions::dateFormat($dateStart)." đến ngày ".ReportFunctions::dateFormat($dateEnd)."", 0, 1, "C");
            ...
            //Table
                ...
                for($i = 1; $i <= sizeof($weeks); $i++) {
                    ...
                    $pdf->Cell(50, 10, "$orderProductCount", 1, 0, 'R');
                    $pdf->Cell(40, 10, "".ReportFunctions::priceFormat($orderTotal)." đ", 1, 0, 'R');
                    $pdf->Cell(40, 10, "".ReportFunctions::priceFormat($grnTotal)." đ", 1, 0, 'R');
                    $pdf->Cell(40, 10, "".ReportFunctions::priceFormat($profit)." đ", 1, 1, 'R');                        
                }
                ...
                $pdf->Cell(40, 12, "".ReportFunctions::priceFormat($sumOrderTotal)." đ", 1, 0, 'R', false);
                $pdf->Cell(40, 12, "".ReportFunctions::priceFormat($sumGRNTotal)." đ", 1, 0, 'R', false);
                $pdf->Cell(40, 12, "".ReportFunctions::priceFormat($sumProfit)." đ", 1, 1, 'R', false);
                //End: table data
            //End: table

            // Price in words
            $pdf->SetFont("DejaVu", "I", 15);
            $pdf->Cell(275, 10, "(".ReportFunctions::numberToWords($sumProfit)." đồng)", 0, 1, 'R');
...
else if (isset($_GET['time']) && ($_GET['time'] == "year") && isset($_GET['report'])) {
    //Define a new A4 PDF object
    ...
    switch ($opt) {
        case "income":
            ...
            for ($i = 1; $i <= sizeof($months); $i++) {
                ...
                /* orderCount */
                $orderCount = Order::getOrderCount($dateMonthStart, $dateMonthEnd);
                /* orderCount */

                /* orderProductCount */
                $orderProductCount = Order::getOrderProductCount($dateMonthStart, $dateMonthEnd);
                /* orderProductCount */

                /* orderTotal */
                $orderTotal = Order::getOrderTotal($dateMonthStart, $dateMonthEnd);
                /* orderTotal */
                ...
                $pdf->SetFont("Times New Roman", "", 20);
                $pdf->Cell(70, 13, "".ReportFunctions::dateFormat($dateMonthStart)."", 1, 0, 'C');
                $pdf->Cell(70, 13, "".ReportFunctions::dateFormat($dateMonthEnd)."", 1, 0, 'C');
                $pdf->Cell(50, 13, "$orderCount", 1, 0, 'R');
                $pdf->Cell(60, 13, "$orderProductCount", 1, 0, 'R');
                $pdf->Cell(120, 13, "".ReportFunctions::priceFormat($orderTotal)." đ", 1, 1, 'R');
            }
            ...
            $pdf->Cell(60, 15, "$sumOrderProductCount", 1, 0, 'R', false);
            $pdf->Cell(120, 15, "".ReportFunctions::priceFormat($sumOrderTotal)." đ", 1, 1, 'R', false);
            //End: table data

            // Price in words
            $pdf->SetFont("Times New Roman", "I", 22);
            $pdf->Cell(0, 15, "(".ReportFunctions::numberToWords($sumOrderTotal)." đồng)", 0, 1, 'R');
        ...
        case "cost":
            ...
            //Title
            $pdf->Cell(0, 10, "BÁO CÁO NHẬP KHO NĂM $year", 0, 1, "C");
            $pdf->SetFont("Times New Roman", "", 20);
            $pdf->Cell(0, 10, "từ ngày $dateStart đến ngày $dateEnd", 0, 1, "C");
            ...
            for ($i = 1; $i <= sizeof($months); $i++) {
                ...
                $dateMonthEnd = $months[$i][sizeof($months[$i]) - 1];
                /* grnCount */
                $grnCount = GRN::getGRNCount($dateMonthStart, $dateMonthEnd);
                /* grnCount */

                /* grnQty */
                $grnQty = GRN::getGRNProductCount($dateMonthStart, $dateMonthEnd);
                /* grnQty */
                
                /* grnTotal */
                $grnTotal = GRN::getGRNTotal($dateMonthStart, $dateMonthEnd);
                /* grnTotal */
                ...
                $pdf->Cell(120, 13, "".ReportFunctions::priceFormat($grnTotal)." đ", 1, 1, 'R');
            }
            ...
            $pdf->Cell(120, 15, "".ReportFunctions::priceFormat($sumGRNTotal)." đ", 1, 1, 'R', false);
            //End: table data

            // Price in words
            $pdf->SetFont("Times New Roman", "I", 20);
            $pdf->Cell(0, 15, "(".ReportFunctions::numberToWords($sumGRNTotal)." đồng)", 0, 1, 'R');
        ...
        case "profit":
            ...
            //Title
            $pdf->Cell(0, 10, "BÁO CÁO LỢI NHUẬN NĂM $year", 0, 1, "C");
            $pdf->SetFont("Times New Roman", "", 20);
            $pdf->Cell(0, 10, "từ ngày $dateStart đến ngày $dateEnd", 0, 1, "C");
            ...
            for ($i = 1; $i <= sizeof($months); $i++) {
                ...
                $pdf->Cell(50, 13, "$orderCount", 1, 0, 'R');
                $pdf->Cell(60, 13, "$orderProductCount", 1, 0, 'R');
                $pdf->Cell(60, 13, "".ReportFunctions::priceFormat($orderTotal)." đ", 1, 0, 'R');
                $pdf->Cell(60, 13, "".ReportFunctions::priceFormat($grnTotal)." đ", 1, 0, 'R');
                $pdf->Cell(60, 13, "".ReportFunctions::priceFormat($profit)." đ", 1, 1, 'R');                                        
            }
            ...
            $pdf->Cell(60, 15, "".ReportFunctions::priceFormat($sumOrderTotal)." đ", 1, 0, 'R', false);
            $pdf->Cell(60, 15, "".ReportFunctions::priceFormat($sumGRNTotal)." đ", 1, 0, 'R', false);
            $pdf->Cell(60, 15, "".ReportFunctions::priceFormat($sumProfit)." đ", 1, 1, 'R', false);
            //End: table data

            // Price in words
            $pdf->SetFont("Times New Roman", "I", 20);
            $pdf->Cell(0, 15, "(".ReportFunctions::numberToWords($sumProfit)." đồng)", 0, 1, 'R');
