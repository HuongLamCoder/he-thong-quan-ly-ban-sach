...
WITH order_by_week AS (SELECT YEARWEEK(ngaytao, 1) AS week_number, -- Xác định tuần
...
GROUP BY YEARWEEK(ngaytao, 1)),
    weekly_comparison AS (SELECT current_week.order_count AS current_week_orders, -- Số đơn hàng tuần này
    prev_week.order_count    AS previous_week_orders -- Số đơn hàng tuần trước
FROM (SELECT * FROM order_by_week ORDER BY week_number DESC LIMIT 1) AS current_week
    LEFT JOIN
...
WITH order_by_week AS (SELECT YEARWEEK(ngaytao, 1) AS week_number, -- Xác định tuần
    ...
    AND idTT = 5                               -- Chỉ lấy đơn hàng đã hoàn thành
GROUP BY YEARWEEK(ngaytao, 1)),
    weekly_comparison AS (SELECT current_week.order_total AS current_week_total, -- Số đơn hàng tuần này
    prev_week.order_total    AS previous_week_total -- Số đơn hàng tuần trước
FROM (SELECT * FROM order_by_week ORDER BY week_number DESC LIMIT 1) AS current_week
    LEFT JOIN
    (SELECT * FROM order_by_week ORDER BY week_number DESC LIMIT 1 OFFSET 1) AS prev_week
ON current_week.week_number - 1 = prev_week.week_number)
SELECT current_week_total,
       previous_week_total
FROM weekly_comparison;

-- Lấy tổng sản phẩm đã bán theo tuần
WITH order_by_week AS (SELECT YEARWEEK(ngaytao, 1) AS week_number,   -- Xác định tuần
    ...
    AND idTT = 5                               -- Chỉ lấy đơn hàng đã hoàn thành
GROUP BY YEARWEEK(ngaytao, 1)),
    weekly_comparison AS (SELECT current_week.quantity_total AS current_week_quantity, -- Số đơn hàng tuần này
    prev_week.quantity_total    AS previous_week_quantity -- Số đơn hàng tuần trước
FROM (SELECT * FROM order_by_week ORDER BY week_number DESC LIMIT 1) AS current_week
    LEFT JOIN
    (SELECT * FROM order_by_week ORDER BY week_number DESC LIMIT 1 OFFSET 1) AS prev_week
ON current_week.week_number - 1 = prev_week.week_number)
SELECT current_week_quantity,
       previous_week_quantity
FROM weekly_comparison;

-- Doanh thu - Chi phí - Lợi nhuận năm
WITH revenue AS (SELECT MONTH(ngaytao)  AS thang,
...
-- Số lượng sách bán được theo thể loại trong năm
SELECT
